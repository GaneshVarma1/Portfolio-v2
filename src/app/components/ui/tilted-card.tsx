...
export const TiltedCard: FC<TiltedCardProps> = ({
  ...
  return (
    <figure
      ref={ref}
      className={`relative [perspective:800px] flex flex-col items-center justify-center border-0 ${className}`}
      style={{
    ...
    >
      ...
      <motion.div
        className="relative [transform-style:preserve-3d] border-0"
        style={{
          width: imageWidth,
          height: imageHeight,
          rotateX,
      ...
      {showTooltip && captionText && (
        <motion.figcaption
          className={`pointer-events-none absolute left-0 top-0 rounded-[4px] 
                     ...
                     opacity-0 z-[3] hidden sm:block shadow-md border-0
                     ${tooltipClassName}`}
          style={{
            x, y, opacity,
